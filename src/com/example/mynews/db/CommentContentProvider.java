...
public class CommentContentProvider extends ContentProvider{
	...
	private SqlCommentHelper dbHelper = null;
	//定义一个UriMatcher对象
	private static final UriMatcher MATCHER = new UriMatcher(UriMatcher.NO_MATCH);
	private static final int NEW_TABLE = 1;
	
	static {
		MATCHER.addURI("com.example.mynews.db", "comments_table", NEW_TABLE);
	}
	...
	public Uri insert(Uri uri, ContentValues values) {
		...
		switch (MATCHER.match(uri)){
			case NEW_TABLE:
				long rowId = db.insert("comments_table", "", values);
	...
	public boolean onCreate() {
		...
		dbHelper = new SqlCommentHelper(this.getContext(),"NEWS.db",null,1);
		return false;
	}
